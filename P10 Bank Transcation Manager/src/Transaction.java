...
public class Transaction implements Comparable<Transaction> {
	...
	public int compareTo(Transaction other) {
      if (priority.compareTo(other.priority) == 0) {
        return (int) (this.user.getBalance() - other.user.getBalance());
      } ;
	...
	public Transaction(Account user, double amount, Type type) {
	  ...
	  this.type=type;
	  switch(type) {
	    case DEPOSIT: priority = Priority.HIGH;
          break;
	    case WITHDRAWAL: priority = Priority.NORMAL;
          break;
	    case LOAN_APPLICATION: if (amount > 3*user.getBalance()) priority = Priority.LOW;
          else
